...
* Leftover golden-path `perceive -C -d 8` then `scroll` still treats a
  ...
  accessible names; unlabeled tag-only labels (`div` / `img` / unnamed
  `link`) do not occupy the 4-sample cap. Leftover-ax-scroll receipts
  print Next `perceive -C -d 8` without Hint `--since-action`. Headline
  membership counts still include unlabeled controls. A new file,
