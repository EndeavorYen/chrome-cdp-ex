...
The current checked boundary has 49 daemon groups, twenty-eight
application-owned handlers, and 40 remaining switch-branch deletion
candidates.
...
six accepted read cohorts, and the first action cohort execute application
handlers, while the other 53 catalog commands return an explicit
