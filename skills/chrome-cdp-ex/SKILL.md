...
4. **Act:** `click`, `fill`, `press`, `select`, `scroll`, or `dismiss-modal` using a fresh `@ref` or stable selector. After fill, `press Enter` with a visible listing link (`See N model results`, `a[href*="models?search="]`, `/search?q=`) submits that listing, not the typeahead first repo. A miss on the realistic mouse path still counts if the listing URL already loaded, otherwise `press` uses existing `jsclick` — it does not send Enter. `perceive -C -d 8` ranks that results link first in Visible controls so `click` can hit it. For a named in-viewport link, `jsclick <target> "Browse 2M+ models"` or `click` with that name (one-step jsclick, skinny URL receipt) instead of `perceive -C -d 8` then `click @ref`. Do not scroll to find it. Mouse `click @ref` still fail-closes with `no-input-events`. For the window document or nested overflow edge, use `scroll to bottom` / `scroll to top` (skinny `scrollY` or `scrollTop` / `scrollMax` / at-bottom or at-top receipt) instead of `perceive` then guessing `scroll down 10000`. When the document cannot scroll, the same commands scroll the nearest overflow container (optional `--scroll-container`) instead of reporting document `0 / 0`. Relative `scroll down N` still settle-diffs. No-op `press Escape`/`Tab`/`Space` and `dismiss-modal` with no dialog are expected no-change / continue — do not send the key name to `overlay`.
