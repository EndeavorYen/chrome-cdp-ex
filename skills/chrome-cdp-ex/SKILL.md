...
4. **Act:** `click`, `fill`, `press`, `select`, `scroll`, or `dismiss-modal` using a fresh `@ref` or stable selector. For the window document edge, use `scroll to bottom` / `scroll to top` (skinny `scrollY` / `scrollMax` / at-bottom or at-top receipt) instead of `perceive` then guessing `scroll down 10000`. Relative `scroll down N` still settle-diffs. Nested overflow is a different job. No-op `press Escape`/`Tab`/`Space` and `dismiss-modal` with no dialog are expected no-change / continue — do not send the key name to `overlay`.
