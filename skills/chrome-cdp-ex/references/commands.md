...
1. **Daily browser attach** — run `cdp doctor` then `cdp list`. Doctor probes `127.0.0.1:9222` (and `CDP_LAST_PORT` if set) before failing. A live debug Chrome on that port is success; do not kill or respawn it. Do not start by clicking `chrome://inspect`.
