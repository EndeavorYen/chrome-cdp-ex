...
- **`-C` visible controls**: a short capped list **after** the article body. `--last` / `--adaptive` apply to this dump. Nav chrome must not outrank the article. After a search-box fill, a typeahead listing link (`See N model results`, `a[href*="models?search="]`, `/search?q=`) is ranked first so truncation cannot hide it; `press Enter` on that search box clicks the listing instead of the first typeahead repo.
