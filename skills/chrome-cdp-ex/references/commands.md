...
> Chrome PDF plugin tabs (`document.contentType` = `application/pdf`) return `chrome-cdp-ex.pdf-viewer.v1` from `perceive`, `text --auto`, `html`, `qa`, `report`, `report --qa`, `click --qa`, `visual-check` / `responsive-audit`, `snap`, `styles`, and other PDF-plugin action receipts. Next is `cdp eval <prefix> "document.contentType"` — do not retry `perceive` / `text` or `click @ref`.
