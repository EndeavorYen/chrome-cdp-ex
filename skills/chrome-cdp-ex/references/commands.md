...
2. **Enable daily-profile debug** — if 9222 is empty, `cdp spawn-debug-browser <preferredBrowser> --daily-profile --port 9222` relaunches that browser's default user-data-dir with `--remote-debugging-port` (ask first; this may quit a running daily browser). `Opening in existing browser session` / `在現有的瀏覽器工作階段中開啟` is not attach success. Chrome 136+ / Edge ignore `--remote-debugging-port` on the default user-data-dir; quit+relaunch of that same default profile may still leave 9222 empty. Use doctor's `preferredBrowser` (macOS default HTTP handler when Edge/Chrome/Brave; not hardcoded chrome or edge). Isolated spawn is not this path. `spawn-debug-browser --help` and `help spawn-debug-browser` print help and must not launch a browser.
