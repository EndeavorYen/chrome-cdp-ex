...
2. **Discover/open:** `cdp list`. Doctor attaches to a daily browser already listening on `9222`. Isolated `cdp spawn-debug-browser` is fallback only and is not the daily profile. If list is empty after attach, `cdp open <url>`. When multiple tabs exist, prefer `cdp target --url <substring>` / `cdp target --title <text>` or `cdp open <url> --reuse-url`. Use `cdp use <target> --name app` when the same live tab will be reused.
...
1. **Daily browser attach** — run `cdp doctor` then `cdp list`. Doctor probes `127.0.0.1:9222` (and `CDP_LAST_PORT` if set) before failing. A live **daily** debug browser on that port is success; do not kill or respawn it. A leftover isolated `chrome-cdp-ex-*` profile on 9222 is not daily attach success — next probe is `--daily-profile` (ask first); do not kill the occupant without asking. Do not start by clicking `chrome://inspect`.
2. **Enable daily-profile debug** — if 9222 is empty, `cdp spawn-debug-browser <preferredBrowser> --daily-profile --port 9222` targets that browser's default user-data-dir (ask first). `Opening in existing browser session` / `在現有的瀏覽器工作階段中開啟` is not attach success. Chrome 136+ / Edge ignore `--remote-debugging-port` on the default user-data-dir; `--daily-profile` does not quit+relaunch that same default profile as if CDP would appear. On 136+, `--daily-profile` opens `chrome://inspect/#remote-debugging` / `edge://inspect/#remote-debugging` in the already-running daily browser and waits for Allow remote debugging; isolated spawn is not this path. Use doctor's `preferredBrowser` (macOS default HTTP handler when Edge/Chrome/Brave; not hardcoded chrome or edge). Isolated spawn is not this path. `spawn-debug-browser --help` and `help spawn-debug-browser` print help and must not launch a browser.
