...
1. **Daily browser attach** — run `cdp doctor` then `cdp list`. Doctor probes `127.0.0.1:9222` (and `CDP_LAST_PORT` if set) before failing. A live **tab daemon** (Electron or a previous attach) is already a usable session: do not occupy 9222; use `cdp perceive <prefix>` or `CDP_PORT=<port> cdp doctor`. A live **daily** debug browser on 9222 is success; do not kill or respawn it. A leftover isolated `chrome-cdp-ex-*` profile on 9222 is not daily attach success — next probe is the persistent daily dir (ask first); do not kill the occupant without asking. Do not start by clicking `chrome://inspect`.
